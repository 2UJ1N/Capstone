...
sents = ["변화를 미리 준비했다면 호랑이로 성장하고 그렇지 않다면 고양이로 전락한다는 뜻으로 표현했다. 그런데 몇몇 독자들이 '고양이가 더 좋은 거 아니냐'는 분들도 있더라. 호랑이보다 고양이가 더 좋게 느껴지는 트렌드가 왔구나 느꼈다. 하지만 내년은 호랑이의 해이기 때문에 그렇게..."]
...
for i in keywords:
    k = i[0].split("/")[0]
    k_val = round(round(i[1],2) * 100)
    k_list = [k,k_val]
    if(len(str(k_list[0]))> 1):
        keywords_l.append(k_list)
